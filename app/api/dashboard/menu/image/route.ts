...
// 메뉴 이미지 업로드 응답 DTO
export interface MenuImageUploadResponseDto {
  imageUrl: string;
}
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        {
          success: false,
          errorMessage: fullMessage || `메뉴 이미지 업로드 실패: ${response.statusText}`,
        },
    ...
    return NextResponse.json(
      {
        success: false,
        errorMessage: `메뉴 이미지 업로드 중 오류: ${errorMessage}`,
      },
      { status: 500 }
    );
  }
