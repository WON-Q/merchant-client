...
const monthlyAlbaData = [
  ...
  { name: "신희원", time: "18:00 ~ 22:00", status: "미출근", payDay: "6월 30일", dDay: "D-23", payInfo: { hours: 24, wage: 9840, totalPay: 236160 } },
    ...
    <DashboardCard className={`relative flex flex-col gap-4 py-5 px-6 ${className}`}>
      {/* 상단 탭 */}
        <div className="absolute top-4 right-4 flex gap-2">
          <Button
            variant="primary" // 항상 primary 스타일로
            size="lg"
              shape="pill"
            className="pointer-events-none" // 혹시 hover 효과 제거
          >
            D-23일
          </Button>
        </div>
          ...
          >
            ...
            <div className="flex justify-between items-center">
             <div className="flex items-center gap-2">
                <span className="text-xl font-bold text-gray-900">{alba.name}
                  <div className="w-full border-t"></div>
                </span>
                {/* <span className="px-2 py-0.5 rounded-full text-m font-bold bg-[#FFF1EC] text-[#FF6B35] border border-[#FF6B35]">
                  {alba.dDay}
                </span> */}
              </div>
...
<div className="flex justify-between text-xl text-gray-700 mt-2 ">
  <div className="flex flex-col gap-y-2">
    ...
    <span className="font-bold text-black">{alba.payInfo.hours}시간</span>
  </div>
  <div className="flex flex-col gap-y-2">
    <span className="text-xl font-semibold text-gray-400">시급</span>
    <span className="font-bold  text-black">{alba.payInfo.wage.toLocaleString()}원</span>
  </div>
  <div className="flex flex-col gap-y-2">
